...
public class Pokemon
{
    ...
    public int Defense { get { return GetStat(Stat.·ÀÓù); } }
    public int SpAttack { get { return GetStat(Stat.ÌØ¹¥); } }
    public int SpDefense { get { return GetStat(Stat.ÌØ·À); } }
    public int Speed { get { return GetStat(Stat.ËÙ¶È); } }
    ...
    private void CalculateStats()
    {
        ...
        {
            { Stat.¹¥»÷, Mathf.FloorToInt(pokemonBase.Attack * Level / 100f) + Random.Range(-5, 6) },
            { Stat.·ÀÓù, Mathf.FloorToInt(pokemonBase.Defense * Level / 100f) + Random.Range(-5, 6) },
            { Stat.ÌØ¹¥, Mathf.FloorToInt(pokemonBase.SpAttack * Level / 100f) + Random.Range(-5, 6) },
            { Stat.ÌØ·À, Mathf.FloorToInt(pokemonBase.SpDefense * Level / 100f) + Random.Range(-5, 6) },
            { Stat.ËÙ¶È, Mathf.FloorToInt(pokemonBase.Speed * Level / 100f) + Random.Range(-5, 6) }
        };
    ...
    public void ApplyBoosts(List<StatBoost> statBoosts)
    {
        foreach (var statBoost in statBoosts)
        {
            ...
            if (boost > 0)
            {
                StatusChanges.Enqueue($"{pokemonBase.PokemonName}µÄ{stat}ÉÏÉýÁË£¡");
            }
            else
            {
